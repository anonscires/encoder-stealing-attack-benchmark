...
from src.dataset.contrastive_learning_dataset import ContrastiveLearningDataset
...
parser.add_argument('--datadir', metavar='DIR', default='./checkpoint/user/data',
...
def main():
    ...
    dataset = ContrastiveLearningDataset(args.datadir)
    ...
    if args.arch in ['vitb16', 'vits16']:
        model = DinoEncoder(base_model=args.arch, out_dim=args.out_dim, loss=args.losstype, entropy=args.entropy, pretrained=False)
    else:
        model = ResNetEncoder(base_model=args.arch, out_dim=args.out_dim, entropy=args.entropy)
