...
if __name__ == '__main__':
    ...
    args.data_dir = f'./datadir/RDA/downstreamdataset/{args.dataset}/'
    ...
    encoder_state_dict = torch.load(args.encoder, map_location='cuda:0')
    encoder = ResNetEncoder(base_model='resnet34', out_dim=512, loss='infonce', include_mlp = False).to('cuda:0')
    encoder.load_state_dict(encoder_state_dict['state_dict'], strict=True)

    feature_bank_training, label_bank_training = predict_feature(encoder, train_loader)
    feature_bank_testing, label_bank_testing = predict_feature(encoder, test_loader)
