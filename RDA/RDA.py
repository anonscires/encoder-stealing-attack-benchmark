...
from src.RDA.models import get_encoder_architecture, get_surrogate_model
...
class OurLoss(nn.Module):
    ...
    def __init__(self, batch_size, temperature=0.07, alpha=1.0, beta=1.0):
...
def steal_train(args, query_data_loader, prototype, surrogate_model, steal_optimizer, loss_func, epoch):
    ...
    print("*"*15)
    print("Text guided stealing is on")
    print("*"*15)
    # Load class mapping
    class_mapping_path = '' #TODO: provide the path to the class mapping file for the query dataset
    ...
    clip_text_embeds = torch.load(embed_path)



    for batch, (img, label) in tqdm(enumerate(query_data_loader), desc='Training process of epoch [{}/{}]'.format(epoch, args.epochs)):
        
        # print('img type:', type(img), 'img length:', len(img), 'img[0] shape:', img[0].shape)
        # print('label type:', type(label), 'label shape:', label.shape)
        ...
        # loading clip text embeddings
        for i, l in enumerate(label):
            if i == 0:
                text_protos = torch.unsqueeze(clip_text_embeds[class_mapping[int(l)]], dim=0)
            else:
                text_protos = torch.cat((text_protos, torch.unsqueeze(clip_text_embeds[class_mapping[int(l)]], dim=0)))

        # if batch == 0 and epoch == 1:
        #     print(protos.shape, text_protos.shape, len(surrogate_embedding_list), surrogate_embedding_list[0].shape, label.shape)
        ...
        for i in range(len(surrogate_embedding_list)):
            if args.loss_function == 'mse':
                ...
                loss += loss_function(torch.squeeze(surrogate_embedding_list[i]), protos.to('cuda'))
                # text loss
                loss += loss_function(torch.squeeze(surrogate_embedding_list[i]), text_protos.to('cuda'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--proto_patch', default=10, type=int, help='patches used for generating prototypical representations')
    parser.add_argument('--query_patch', default=5, type=int, help='training paths')
    ...
    parser.add_argument('--target_check_point', default='results/', type=str, help='path to the check point of the target model')
    ...
    if args.pretraining_dataset in ['cifar10', 'stl10', 'svhn']:
        out_dim = 512
        if args.victim_path is None:
            raise Exception('Please provide the path to the victim model.')
        target_model = ResNetEncoder(base_model='resnet34',
                                            out_dim=512, loss='infonce', include_mlp=False).to('cuda')
        target_model = load_victim(args.victim_path, target_model, device='cuda', discard_mlp=True)
    elif args.pretraining_dataset in ['clip']:
        target_model = CLIPVisionEncoder(base_model='clip-vitb16', out_dim=512, include_mlp=False, pretrained=True).cuda()
    elif args.pretraining_dataset in ['imagenet', ]:
        raise NotImplementedError('The encoder of ImageNet pretraining is not implemented yet.')
    ...
    if args.architecture == 'resnet50':
        surrogate_model = ResNetEncoder(base_model=args.architecture, out_dim=512, include_mlp=True).cuda()
    else:
        surrogate_model = ResNetEncoder(base_model=args.architecture, out_dim=512, include_mlp=False).cuda()

    # load the query dataset and proto dataset
    args.query_data_dir = f'./datadir/RDA/downstreamdataset/{args.query_dataset}/query/'
    args.test_data_dir = f'./datadir/RDA/downstreamdataset/{args.target_downstream_dataset}/'
    ...
    memory_loader = DataLoader(memory_data, batch_size=args.query_batch_size, shuffle=False,
                               num_workers=12,
                               pin_memory=True)

    test_loader = DataLoader(test_data, batch_size=args.query_batch_size, shuffle=False,
                             num_workers=12,
                             pin_memory=True)
    ...
    # else:
    #     _ = test(target_model, memory_loader, test_loader, 0, args)  # CIFAR10 Pre-trained: CIFAR10: 88.31%  STL10: 71.59%  ImageNet Pre-trained: CIFAR10:81.21% STL10: 71.99 (112*112) CLIP: stl10: 74.09 (224*224)
    ...
    for epoch in range(epoch_start, args.epochs+1):
        ...
        if epoch % args.test_freq == 0:
            ...
            data_frame.to_csv(args.log_results_dir + '/test_log_seed{}.csv'.format(args.query_patch, args.seed), index_label='epoch')

            if test_acc >= best_acc:
                best_acc = test_acc
                torch.save({'state_dict': surrogate_model_new.state_dict()}, args.model_results_dir + '/our_model_seed{}_'.format(args.seed)+ args.query_dataset + '.pth')
