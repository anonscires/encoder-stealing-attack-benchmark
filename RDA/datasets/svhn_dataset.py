...
def get_pretraining_svhn(data_dir):
    ...
    train_data = DATAPAIR(numpy_file=data_dir + "train.npz", class_type=svhn_classes, transform=train_transform)
    memory_data = TESTDATA(numpy_file=data_dir + "train.npz", class_type=svhn_classes, transform=test_transform_cifar10)
    test_data  = TESTDATA(numpy_file=data_dir + "test.npz", class_type=svhn_classes,transform=test_transform_cifar10)
...
def get_downstream_svhn(args):
    ...
    downstream_training_data = TESTDATA(numpy_file=args.data_dir+training_file_name, class_type=svhn_classes, transform=test_transform)
    testing_data = TESTDATA(numpy_file=args.data_dir+testing_file_name, class_type=svhn_classes, transform=test_transform)
