...
def get_pretraining_stl10(data_dir):
    ...
    train_data = DATAPAIR(numpy_file=data_dir + "train_unlabeled.npz", class_type= stl10_classes, transform=train_transform)
    memory_data = TESTDATA(numpy_file=data_dir + "train.npz", class_type=stl10_classes, transform=test_transform_stl10)
    test_data  = TESTDATA(numpy_file=data_dir + "test.npz", class_type= stl10_classes,transform=test_transform_stl10)
...
def get_downstream_stl10(args):
    ...
    downstream_training_data = TESTDATA(numpy_file=args.data_dir+training_file_name, class_type=stl10_classes, transform=test_transform)
    testing_data = TESTDATA(numpy_file=args.data_dir+testing_file_name, class_type=stl10_classes, transform=test_transform)
